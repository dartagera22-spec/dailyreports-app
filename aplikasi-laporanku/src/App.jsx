...
// 🛠️ PERBAIKAN SAKTI DHELILA: Inisialisasi Ganda (Canvas & Vite) yang Anti-Crash
try {
  const getEnv = (key) => {
  ...
  if (typeof __firebase_config !== 'undefined') {
    ...
    if (apiKey) {
      ...
      appId = getEnv('VITE_FIREBASE_PROJECT_ID') || 'default-app-id';
  ...
  // Aktifkan Mode Offline jika Database berhasil terhubung
  if (db) {
    try {
    ...
    console.warn("⚠️ Firebase tidak terinisialisasi. Periksa konfigurasi .env atau Canvas.");
  }
} catch (error) {
...
const StoreProvider = ({ children }) => {
  ...
  useEffect(() => {
    // 🛠️ PERBAIKAN SAKTI: Guard agar tidak crash jika Firebase gagal dimuat
    if (!auth) return;
  ...
  useEffect(() => {
    // 🛠️ PERBAIKAN SAKTI: Guard untuk database
    if (!user || !db) return;
      ...
      <div className="px-6 mt-6 pb-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors duration-300">
          <div className="w-full flex items-center justify-between p-4 border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition cursor-pointer active:bg-slate-100 dark:active:bg-slate-700" onClick={() => { setSaveToGallery(!saveToGallery); triggerToast(!saveToGallery ? 'Fitur simpan ke Galeri HP diaktifkan! 📱' : 'Penyimpanan ke Galeri HP dimatikan.'); }}>
            ...
            <button className={`w-10 h-5 rounded-full relative transition-colors duration-300 shrink-0 ${saveToGallery ? 'bg-emerald-500 shadow-inner' : 'bg-slate-300 dark:bg-slate-600 shadow-inner'}`}><div className={`w-4 h-4 bg-white rounded-full absolute top-0.5 transition-transform duration-300 ease-out shadow-sm ${saveToGallery ? 'translate-x-5' : 'translate-x-1'}`}></div></button>
          </div>
...
// 🛠️ PERBAIKAN SAKTI DHELILA: Mengisolasi Draft Form agar lebih Cepat (Debounce)
const NewReportView = () => {
  ...
  // Local State untuk performa yang jauh lebih ringan saat mengetik
  const [localDraft, setLocalDraft] = useState(draftReport);

  // Sync dari Local ke Global setiap 800ms jika ada perubahan (Debounce)
  useEffect(() => {
    const timer = setTimeout(() => {
      setDraftReport(localDraft);
    }, 800);
    return () => clearTimeout(timer);
  }, [localDraft, setDraftReport]);
  ...
  const updateDraft = (key, value) => { setLocalDraft(prev => ({ ...prev, [key]: value })); if (formErrors[key]) setFormErrors(prev => ({ ...prev, [key]: false })); };
  ...
  const handleManpowerChange = (id, field, value) => { const val = field === 'type' ? toTitleCase(value) : value; setLocalDraft(prev => ({ ...prev, manpower: prev.manpower.map(item => item.id === id ? { ...item, [field]: val } : item) })); };
  const addManpower = () => setLocalDraft(prev => ({ ...prev, manpower: [...prev.manpower, { id: Date.now() + Math.random(), type: '', count: '', hours: '' }] }));
  const removeManpower = (id) => setLocalDraft(prev => ({ ...prev, manpower: prev.manpower.filter(item => item.id !== id) }));

  const handleEquipmentChange = (id, field, value) => { const val = field === 'name' ? toTitleCase(value) : value; setLocalDraft(prev => ({ ...prev, equipment: prev.equipment.map(item => item.id === id ? { ...item, [field]: val } : item) })); };
  const addEquipment = () => setLocalDraft(prev => ({ ...prev, equipment: [...prev.equipment, { id: Date.now() + Math.random(), name: '', status: 'active', hours: '' }] }));
  const removeEquipment = (id) => setLocalDraft(prev => ({ ...prev, equipment: prev.equipment.filter(item => item.id !== id) }));

  const handleSaveReport = async () => {
    let errors = { projectName: false, notes: false };
    if (!localDraft.projectName.trim()) errors.projectName = true;
    if (!localDraft.notes.trim()) errors.notes = true;
    setFormErrors(errors);

    if (errors.projectName || errors.notes) { triggerToast('⚠️ Lengkapi kolom yang bergaris merah!'); return; }
    if (!user || !db) { triggerToast('⚠️ Autentikasi Firebase belum siap atau terputus!'); return; }
    ...
    try {
      const newReport = {
        ...
        date: new Date(localDraft.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }),
        status: isOffline ? 'pending' : 'synced', 
        weatherMorning: localDraft.weatherMorning, 
        weatherAfternoon: localDraft.weatherAfternoon,
        notes: localDraft.notes, 
        photos: capturedPhotos, 
        manpower: [...localDraft.manpower], 
        equipment: [...localDraft.equipment],
        createdAt: Date.now()
      };
      
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'reports'), newReport);
      
      if (localDraft.projectName && !projectList.find(p => p.name === localDraft.projectName.trim())) {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'projects'), { 
          id: `P-${Date.now().toString().slice(-4)}`,
          name: localDraft.projectName.trim(), 
      ...
      const resetState = { projectName: '', date: new Date().toISOString().split('T')[0], weatherMorning: 'Cerah', weatherAfternoon: 'Berawan', notes: '', manpower: [], equipment: [] };
      setLocalDraft(resetState);
      setDraftReport(resetState);
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 relative transition-colors duration-300 animate-slide-in-right">
      ...
      <div className="flex-1 overflow-y-auto p-5 pb-28" ref={containerRef} onScroll={handleScroll}>
          ...
          <div className="space-y-3">
            ...
            <div className="relative group cursor-pointer">
              ...
              <div className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 text-sm rounded-lg p-2.5 flex items-center justify-between font-medium text-slate-700 dark:text-slate-200 transition-colors group-hover:border-sky-300">
                {localDraft.date ? new Date(localDraft.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : ''} <Calendar size={16} className="text-slate-400" />
              </div>
              <input type="date" value={localDraft.date} onChange={(e) => updateDraft('date', e.target.value)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer pt-4" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 mb-5 transition-colors duration-300">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-sm rounded-lg p-2.5 focus:ring-sky-500" value={localDraft.weatherMorning} onChange={(e) => updateDraft('weatherMorning', e.target.value)}>
            ...
            <div>
              ...
              <select className="w-full bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-sm rounded-lg p-2.5 focus:ring-sky-500" value={localDraft.weatherAfternoon} onChange={(e) => updateDraft('weatherAfternoon', e.target.value)}>
                <option value="Cerah">☀️ Cerah</option><option value="Berawan">⛅ Berawan</option><option value="Hujan Ringan">🌧️ Hujan Ringan</option><option value="Hujan Lebat">⛈️ Hujan Lebat</option>
              </select>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 mb-5 transition-colors duration-300">
          ...
          <div className="space-y-3">
            {localDraft.manpower.map((worker) => (
      ...
      {showCameraModal && <CameraModal onClose={() => setShowCameraModal(false)} defaultLocation={localDraft.projectName} />}
      {showVoiceNoteModal && <VoiceNoteModal onClose={() => setShowVoiceNoteModal(false)} onSaveTranscript={(text) => updateDraft('notes', localDraft.notes ? localDraft.notes + '\n' + text : text)} />}
    </div>
  );
};
...
// 🛠️ PERBAIKAN SAKTI DHELILA: Mengatur Senter Hardware secara nyata
const CameraModal = ({ onClose, defaultLocation }) => {
  const { projectList, capturedPhotos, setCapturedPhotos, saveToGallery, triggerToast, appSettings } = useStore();
  ...
  const [gpsData, setGpsData] = useState({ lat: 'Mencari...', long: 'Mencari...', location: defaultLocation || 'Lokasi Proyek', address: 'Menunggu sinyal GPS...' });
      ...
      if (track) {
        try {
          ...
          if (capabilities.torch) {
            await track.applyConstraints({ advanced: [{ torch: newMode === 'on' }] });
            triggerToast(`⚡ Flash Kamera: ${newMode.toUpperCase()}`);
          } else {
            triggerToast('⚠️ Senter tidak didukung di perangkat/browser ini.');
            setFlashMode('off');
          }
        } catch (err) {
          console.warn("Flashlight error:", err);
          triggerToast('⚠️ Gagal menyalakan senter. Mungkin izin ditolak.');
          setFlashMode('off');
        }
      }
    } else {
      triggerToast(`⚡ Flash Kamera disiapkan ke: ${newMode.toUpperCase()}`);
    }
  };
...
// 🛠️ PERBAIKAN SAKTI DHELILA: Mengamankan Fitur Suara dari Browser Lawas
const VoiceNoteModal = ({ onClose, onSaveTranscript }) => {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      triggerToast('Maaf, browser/perangkat Anda tidak mendukung fitur Dikte Suara. 😔');
    }
  ...
  const toggleListen = () => {
    if (!isSupported) {
      triggerToast('Dikte suara tidak didukung browser ini. Silakan ketik manual.');
      return;
    }
    ...
    if (isListening) {
      ...
      try {
        ...
        console.error(e);
        triggerToast('Gagal memulai mikrofon.');
      }
    }
  };
  ...
  return (
    <div className="absolute inset-0 z-[200] flex flex-col h-full bg-slate-900 text-white items-center animate-fade-in-up">
      ...
      <div className="flex flex-col items-center mt-24 mb-8">
        <button onClick={toggleListen} disabled={!isSupported} className={`w-28 h-28 rounded-full flex items-center justify-center border-4 transition-all duration-500 ${!isSupported ? 'bg-slate-800/50 border-slate-700/50 opacity-50 cursor-not-allowed' : isListening ? 'bg-red-500/20 border-red-500/50 animate-pulse scale-110 shadow-[0_0_30px_rgba(239,68,68,0.4)]' : isFinished ? 'bg-emerald-500/10 border-emerald-500/30 hover:bg-emerald-500/20 active:scale-95' : 'bg-slate-800 border-slate-700 hover:border-purple-500 hover:bg-slate-800/80'}`}>
        ...
        <p className={`text-xs font-semibold uppercase tracking-wider mt-6 text-center px-4 ${!isSupported ? 'text-red-400' : isListening ? 'text-red-400 animate-pulse' : isFinished ? 'text-emerald-300' : 'text-slate-400'}`}>
          {!isSupported ? 'Fitur Tidak Didukung Perangkat Ini' : isListening ? 'Sedang Merekam (Ketuk untuk Stop)' : isFinished ? 'Selesai (Ketuk Ulang Merekam)' : 'Ketuk Mikrofon Untuk Mulai'}
        </p>
      </div>
      ...
      <div className="absolute bottom-12 w-full px-8">
        <button onClick={() => { onSaveTranscript(transcript); onClose(); }} disabled={!transcript || isListening} className={`w-full font-bold text-sm py-4 rounded-xl shadow-lg transition-all flex items-center justify-center ${transcript && !isListening ? 'bg-purple-600 text-white hover:bg-purple-700 shadow-[0_0_20px_rgba(147,51,234,0.4)]' : 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'}`}><CheckCircle2 size={18} className="mr-2" /> Gunakan Teks</button>
      </div>
    </div>
  );
};
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 relative justify-center px-8 animate-fade-in overflow-hidden transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-700 relative z-10 animate-fade-in-up">
        ...
        <form onSubmit={async (e) => { 
          ...
          if (!isMountedRef.current) return;
          // 🛠️ PERBAIKAN SAKTI: Tambahan guard agar jika sistem database rusak, ia tidak terproses
          if (!db || !auth) {
            triggerToast('⚠️ Sistem belum terhubung ke database. Harap muat ulang!');
          ...
          try {
            ...
            // 🛡️ MENYIMPAN PROFIL KE FIREBASE AUTH (TAHAN BANTING DARI RELOAD)
            if (auth.currentUser) {
...
// ==========================================
// 📈 ADMIN DASHBOARD VIEW (Dengan Manajemen Lisensi)
// ==========================================
const AdminDashboardView = () => {
  const { reports, projectList, setIsAuthenticated, triggerToast, setCurrentPage, accessCodesList, userProfile } = useStore();
  
  // TAHAP 1 (Opsi 1): Navigasi Tab Admin
  const [adminTab, setAdminTab] = useState('overview'); // 'overview' atau 'licenses'
  ...
  // 🛡️ MENGISOLASI DATA (Multi-Tenant Security):
  ...
  const handleCreateLicense = async (e) => {
    ...
    if (!db) { triggerToast('⚠️ Koneksi Database belum siap.'); return; }
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 relative transition-colors duration-300">
      <div className="bg-indigo-600 dark:bg-indigo-800 text-white p-6 rounded-b-3xl shadow-lg transition-colors duration-300">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={async () => { 
            if (auth?.currentUser) await updateProfile(auth.currentUser, { displayName: "" }).catch(()=>{});
            localStorage.removeItem('dr_isAuthenticated');
            setIsAuthenticated(false); 
            setCurrentPage('dashboard'); 
